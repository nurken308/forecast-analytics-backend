...
DAILY_ENTITY_FILTERS = {
    "retail": "m.Segment1 = '4.Розница'",

    "bk": "m.Products = 'БК'",
    "bk_soft": "m.Products = 'БК' AND UPPER(m.ptflo_blng) = 'SOFT'",

    "vkl": "m.Products = 'ВКЛ'",
    "vkl_soft": "m.Products = 'ВКЛ' AND UPPER(m.ptflo_blng) = 'SOFT'",

    "rb_z_ip": "m.Products = 'РБ ЗАЛОГОВЫЙ ИП'",
    "rb_z_ip_soft": "m.Products = 'РБ ЗАЛОГОВЫЙ ИП (с)'",
    "rb_z_too": "m.Products = 'РБ ЗАЛОГОВЫЙ ТОО'",

    "rb_bz_ip": "m.Products = 'РБ БЗ ИП'",
    "rb_bz_too": "m.Products = 'РБ БЗ ТОО'",
}
...
def load_oracle_pivot(segment_id: str, base_ymd: str | None = None):
    ...
    # Daily entities:
    # один общий Oracle-запрос, затем фильтрация нужного продукта
    if segment_id in DAILY_ENTITY_FILTERS:
        df = load_oracle_daily_raw(day).copy()
        df.columns = [column.lower() for column in df.columns]

        if segment_id == "retail":
            mask = df["segment1"] == "4.Розница"

        elif segment_id == "bk":
            mask = df["products"] == "БК"

        elif segment_id == "bk_soft":
            mask = (
                (df["products"] == "БК")
                & (df["ptflo_blng"].fillna("").str.upper() == "SOFT")
            )

        elif segment_id == "vkl":
            mask = df["products"] == "ВКЛ"

        elif segment_id == "vkl_soft":
            mask = (
                (df["products"] == "ВКЛ")
                & (df["ptflo_blng"].fillna("").str.upper() == "SOFT")
            )

        elif segment_id == "rb_z_ip":
            mask = df["products"] == "РБ ЗАЛОГОВЫЙ ИП"

        elif segment_id == "rb_z_ip_soft":
            mask = df["products"] == "РБ ЗАЛОГОВЫЙ ИП (с)"

        elif segment_id == "rb_z_too":
            mask = df["products"] == "РБ ЗАЛОГОВЫЙ ТОО"

        elif segment_id == "rb_bz_ip":
            mask = df["products"] == "РБ БЗ ИП"

        elif segment_id == "rb_bz_too":
            mask = df["products"] == "РБ БЗ ТОО"

        else:
            raise ValueError(f"Unsupported daily segment_id: {segment_id}")

        filtered = df.loc[
            mask,
            ["base_ymd", "od", "prosrochka_1"],
        ].copy()
        ...
        return (
            filtered
            .groupby("base_ymd", as_index=False)[
                ["od", "prosrochka_1"]
            ]
            .sum()
        )
...
ORDER BY m.base_ymd
    ...
    return df
def load_oracle_daily_raw(base_ymd: str):
    day = base_ymd
